/* Handwritten function */
nonmatching func_801C7110, 0x210

glabel func_801C7110
    /* C7110 801C7110 3C018011 */  lui        $at, %hi(D_8010C330)
    /* C7114 801C7114 D420C330 */  ldc1       $fv0, %lo(D_8010C330)($at)
    /* C7118 801C7118 46240002 */  mul.d      $fv0, $fv0, $ft0
    /* C711C 801C711C 3C018011 */  lui        $at, %hi(D_8010C328)
    /* C7120 801C7120 D422C328 */  ldc1       $fv1, %lo(D_8010C328)($at)
    /* C7124 801C7124 46220000 */  add.d      $fv0, $fv0, $fv1
    /* C7128 801C7128 46240002 */  mul.d      $fv0, $fv0, $ft0
    /* C712C 801C712C 3C018011 */  lui        $at, %hi(D_8010C320)
    /* C7130 801C7130 D422C320 */  ldc1       $fv1, %lo(D_8010C320)($at)
    /* C7134 801C7134 46220000 */  add.d      $fv0, $fv0, $fv1
    /* C7138 801C7138 46240002 */  mul.d      $fv0, $fv0, $ft0
    /* C713C 801C713C 00000000 */  nop
    /* C7140 801C7140 46243082 */  mul.d      $fv1, $ft1, $ft0
    /* C7144 801C7144 3C018011 */  lui        $at, %hi(D_8010C318)
    /* C7148 801C7148 D424C318 */  ldc1       $ft0, %lo(D_8010C318)($at)
    /* C714C 801C714C 46240000 */  add.d      $fv0, $fv0, $ft0
    /* C7150 801C7150 46201082 */  mul.d      $fv1, $fv1, $fv0
    /* C7154 801C7154 30620001 */  andi       $v0, $v1, 0x1
    /* C7158 801C7158 14400003 */  bnez       $v0, .L801C7168
    /* C715C 801C715C 46223000 */   add.d     $fv0, $ft1, $fv1
    /* C7160 801C7160 08071866 */  j          .L801C6198
    /* C7164 801C7164 46200020 */   cvt.s.d   $fv0, $fv0
  .L801C7168:
    /* C7168 801C7168 46200020 */  cvt.s.d    $fv0, $fv0
    /* C716C 801C716C 08071866 */  j          .L801C6198
    /* C7170 801C7170 46000007 */   neg.s     $fv0, $fv0
  .L801C7174:
    /* C7174 801C7174 460C6032 */  c.eq.s     $fa0, $fa0
    /* C7178 801C7178 00000000 */  nop
    /* C717C 801C717C 45000004 */  bc1f       .L801C7190
    /* C7180 801C7180 00000000 */   nop
    /* C7184 801C7184 3C018011 */  lui        $at, (0x80110000 >> 16)
    /* C7188 801C7188 08071866 */  j          .L801C6198
    /* C718C 801C718C C420C350 */   lwc1      $fv0, -0x3CB0($at)
  .L801C7190:
    /* C7190 801C7190 3C018011 */  lui        $at, %hi(D_8010C270)
    /* C7194 801C7194 C420C270 */  lwc1       $fv0, %lo(D_8010C270)($at)
    /* C7198 801C7198 03E00008 */  jr         $ra
    /* C719C 801C719C 00000000 */   nop
    /* C71A0 801C71A0 27BDFFD0 */  addiu      $sp, $sp, -0x30
    /* C71A4 801C71A4 F7B40018 */  sdc1       $fs0, 0x18($sp)
    /* C71A8 801C71A8 4485A000 */  mtc1       $a1, $fs0
    /* C71AC 801C71AC F7B60020 */  sdc1       $fs1, 0x20($sp)
    /* C71B0 801C71B0 4486B000 */  mtc1       $a2, $fs1
    /* C71B4 801C71B4 AFB00010 */  sw         $s0, 0x10($sp)
    /* C71B8 801C71B8 F7B80028 */  sdc1       $fs2, 0x28($sp)
    /* C71BC 801C71BC 4487C000 */  mtc1       $a3, $fs2
    /* C71C0 801C71C0 AFBF0014 */  sw         $ra, 0x14($sp)
    /* C71C4 801C71C4 0C071510 */  jal        func_801C5440
    /* C71C8 801C71C8 00808021 */   addu      $s0, $a0, $zero
    /* C71CC 801C71CC E6140030 */  swc1       $fs0, 0x30($s0)
    /* C71D0 801C71D0 E6160034 */  swc1       $fs1, 0x34($s0)
    /* C71D4 801C71D4 E6180038 */  swc1       $fs2, 0x38($s0)
    /* C71D8 801C71D8 8FBF0014 */  lw         $ra, 0x14($sp)
    /* C71DC 801C71DC 8FB00010 */  lw         $s0, 0x10($sp)
    /* C71E0 801C71E0 D7B80028 */  ldc1       $fs2, 0x28($sp)
    /* C71E4 801C71E4 D7B60020 */  ldc1       $fs1, 0x20($sp)
    /* C71E8 801C71E8 D7B40018 */  ldc1       $fs0, 0x18($sp)
    /* C71EC 801C71EC 03E00008 */  jr         $ra
    /* C71F0 801C71F0 27BD0030 */   addiu     $sp, $sp, 0x30
    /* C71F4 801C71F4 27BDFF88 */  addiu      $sp, $sp, -0x78
    /* C71F8 801C71F8 F7B40060 */  sdc1       $fs0, 0x60($sp)
    /* C71FC 801C71FC 4485A000 */  mtc1       $a1, $fs0
    /* C7200 801C7200 F7B60068 */  sdc1       $fs1, 0x68($sp)
    /* C7204 801C7204 4486B000 */  mtc1       $a2, $fs1
    /* C7208 801C7208 AFB10054 */  sw         $s1, 0x54($sp)
    /* C720C 801C720C 00808821 */  addu       $s1, $a0, $zero
    /* C7210 801C7210 AFB00050 */  sw         $s0, 0x50($sp)
    /* C7214 801C7214 27B00010 */  addiu      $s0, $sp, 0x10
    /* C7218 801C7218 F7B80070 */  sdc1       $fs2, 0x70($sp)
    /* C721C 801C721C 4487C000 */  mtc1       $a3, $fs2
    /* C7220 801C7220 AFBF0058 */  sw         $ra, 0x58($sp)
    /* C7224 801C7224 0C071510 */  jal        func_801C5440
    /* C7228 801C7228 02002021 */   addu      $a0, $s0, $zero
    /* C722C 801C722C 02002021 */  addu       $a0, $s0, $zero
    /* C7230 801C7230 02202821 */  addu       $a1, $s1, $zero
    /* C7234 801C7234 E7B40040 */  swc1       $fs0, 0x40($sp)
    /* C7238 801C7238 E7B60044 */  swc1       $fs1, 0x44($sp)
    /* C723C 801C723C 0C0714C0 */  jal        func_801C5300
    /* C7240 801C7240 E7B80048 */   swc1      $fs2, 0x48($sp)
    /* C7244 801C7244 8FBF0058 */  lw         $ra, 0x58($sp)
    /* C7248 801C7248 8FB10054 */  lw         $s1, 0x54($sp)
    /* C724C 801C724C 8FB00050 */  lw         $s0, 0x50($sp)
    /* C7250 801C7250 D7B80070 */  ldc1       $fs2, 0x70($sp)
    /* C7254 801C7254 D7B60068 */  ldc1       $fs1, 0x68($sp)
    /* C7258 801C7258 D7B40060 */  ldc1       $fs0, 0x60($sp)
    /* C725C 801C725C 03E00008 */  jr         $ra
    /* C7260 801C7260 27BD0078 */   addiu     $sp, $sp, 0x78
    /* C7264 801C7264 00000000 */  nop
    /* C7268 801C7268 00000000 */  nop
    /* C726C 801C726C 00000000 */  nop
    /* C7270 801C7270 97A20012 */  lhu        $v0, 0x12($sp)
    /* C7274 801C7274 97A30016 */  lhu        $v1, 0x16($sp)
    /* C7278 801C7278 93A8001B */  lbu        $t0, 0x1B($sp)
    /* C727C 801C727C 93A9001F */  lbu        $t1, 0x1F($sp)
    /* C7280 801C7280 97AA0022 */  lhu        $t2, 0x22($sp)
    /* C7284 801C7284 97AB0026 */  lhu        $t3, 0x26($sp)
    /* C7288 801C7288 AC850000 */  sw         $a1, 0x0($a0)
    /* C728C 801C728C AC860004 */  sw         $a2, 0x4($a0)
    /* C7290 801C7290 A4870008 */  sh         $a3, 0x8($a0)
    /* C7294 801C7294 A482000A */  sh         $v0, 0xA($a0)
    /* C7298 801C7298 A483000C */  sh         $v1, 0xC($a0)
    /* C729C 801C729C A088000E */  sb         $t0, 0xE($a0)
    /* C72A0 801C72A0 A089000F */  sb         $t1, 0xF($a0)
    /* C72A4 801C72A4 A48A0010 */  sh         $t2, 0x10($a0)
    /* C72A8 801C72A8 03E00008 */  jr         $ra
    /* C72AC 801C72AC A48B0012 */   sh        $t3, 0x12($a0)
    /* C72B0 801C72B0 40024800 */  mfc0       $v0, $9 /* handwritten instruction */
    /* C72B4 801C72B4 03E00008 */  jr         $ra
    /* C72B8 801C72B8 00000000 */   nop
    /* C72BC 801C72BC 00000000 */  nop
    /* C72C0 801C72C0 18A00020 */  blez       $a1, .L801C7344
    /* C72C4 801C72C4 00000000 */   nop
    /* C72C8 801C72C8 240B2000 */  addiu      $t3, $zero, 0x2000
    /* C72CC 801C72CC 00AB082B */  sltu       $at, $a1, $t3
    /* C72D0 801C72D0 1020001E */  beqz       $at, .L801C734C
    /* C72D4 801C72D4 00000000 */   nop
    /* C72D8 801C72D8 00804021 */  addu       $t0, $a0, $zero
    /* C72DC 801C72DC 00854821 */  addu       $t1, $a0, $a1
    /* C72E0 801C72E0 0109082B */  sltu       $at, $t0, $t1
    /* C72E4 801C72E4 10200017 */  beqz       $at, .L801C7344
    /* C72E8 801C72E8 00000000 */   nop
    /* C72EC 801C72EC 2529FFF0 */  addiu      $t1, $t1, -0x10
    /* C72F0 801C72F0 310A000F */  andi       $t2, $t0, 0xF
    /* C72F4 801C72F4 11400007 */  beqz       $t2, .L801C7314
    /* C72F8 801C72F8 00000000 */   nop
    /* C72FC 801C72FC 010A4023 */  subu       $t0, $t0, $t2
    /* C7300 801C7300 BD150000 */  cache      0x15, 0x0($t0) /* handwritten instruction */
    /* C7304 801C7304 0109082B */  sltu       $at, $t0, $t1
    /* C7308 801C7308 1020000E */  beqz       $at, .L801C7344
  .L801C730C:
    /* C730C 801C730C 00000000 */   nop
    /* C7310 801C7310 25080010 */  addiu      $t0, $t0, 0x10
  .L801C7314:
    /* C7314 801C7314 312A000F */  andi       $t2, $t1, 0xF
    /* C7318 801C7318 11400006 */  beqz       $t2, .L801C7334
    /* C731C 801C731C 00000000 */   nop
endlabel func_801C7110
